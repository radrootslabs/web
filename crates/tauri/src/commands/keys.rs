...
pub async fn keys_nostr_gen(rr: tauri::State<'_, Radroots>) -> Result<IResult<String>, String> {
    let keys = lib_nostr_keys_gen();
    keystore::key_add(&keys, &rr.data_dir)
...
pub async fn keys_nostr_read(
    ...
    let keys = keystore::key_read(&public_key, &rr.data_dir)
...
pub async fn keys_nostr_read_all(
    ...
    let keystore_keys = keystore::keys_read_all(&rr.data_dir)
...
pub async fn keys_nostr_delete(
    ...
    keystore::key_delete(&public_key, &rr.data_dir)
...
pub async fn keys_nostr_keystore_reset(
    ...
    keystore::reset(&rr.data_dir).map_err(|e| format!("Failed to reset keystore: {}", e))?;
    Ok(IResultPass { pass: true })
}
