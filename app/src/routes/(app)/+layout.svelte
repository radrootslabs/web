<script lang="ts">
    ...
    const nostr_init = async (): Promise<void> => {
        try {
            if (!$key_nostr) return void throw_err(`*-key_nostr`);
            const keys_nostr_read = await keys.nostr_read($key_nostr);
            ...
            await $ndk.connect();
            const _ndk_user = await ndk_init({
                $ndk,
                secret_key: keys_nostr_read.secret_key,
            });
            nostr_ndk_configured.set(!!_ndk_user);
            if (!_ndk_user) return;
            $ndk_user = _ndk_user;
            $ndk_user.ndk = $ndk;
        } catch (e) {
